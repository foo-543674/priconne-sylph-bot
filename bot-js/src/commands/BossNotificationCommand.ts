...
export class BossNotificationCommand implements MessageCommand {
    ...
    async execute(message: Message): Promise<void> {
        ...
        const isBossQuestionnaireMessage = (message: Message) => (
            ...
            && message.cleanContent === this.phraseRepository.get(PhraseKey.bossQuestionnaireMessage())
        )
        ...
        await pipe(
            ...
            TaskOption.chain(bossNumber => pipe(
                ...
                TaskOption.chain(messages => TaskOption.fromNullable(messages.find(isBossQuestionnaireMessage))),
                TaskOption.fold(
